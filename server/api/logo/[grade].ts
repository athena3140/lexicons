export default defineEventHandler((event) => {
	...
	const { grade } = params;
	const xPosition = grade.includes("12%20NUG") ? 1246 : 1171.403;
	const textAnchor = grade.includes("12%20NUG") ? "end" : "middle";
	...
	const svg = `
    		<svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" viewBox="0 0 1246 350">
      		...
      		<text fill="#00FFFF" font-size="60px" text-anchor="${textAnchor}" font-family="Poppins" x="${xPosition}" y="330">
        			<tspan x="${xPosition}">${grade.replace("%20", " ")}</tspan>
      		</text>
    		</svg>
  `;
